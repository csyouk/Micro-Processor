...
//  // 위와 동일함. 단지 분주비만 다름.
//		Macro_Write_Block(rTCFG0, 0xff, (int)(PCLK/50000./8 + 0.5) - 1, 0);
//		Macro_Write_Block(rTCFG1, 0xf, 0x2, 0);
...
void Timer0_Repeat_ISR_Enable(int en, int time)
{
	if(en)
	{
		// 주어진 시간으로 Auto-repeat 모드로 동작하도록 타이머 설정
		Macro_Write_Block(rTCFG0, 0xff, (int)(PCLK/50000./4 + 0.5) - 1, 0);
		Macro_Write_Block(rTCFG1, 0xf, 0x1, 0);

...
extern int red_divider;
extern int green_divider;
extern int blue_divider;
...
void Lcd_Put_Cam_Pixel(int x,int y,unsigned int color)
{
	...
	if(Frame_Mode == LAYER_MODE)
	{
//		Uart_Printf("layer mode\n");
		Fb_Layer[y][3*x] 	= ((color << 8) & (0xf80000))/red_divider;     // R
		Fb_Layer[y][3*x+1] = ((color << 13) & (0xfc0000))/green_divider;    // G
		Fb_Layer[y][3*x+2] = ((color << 19) & (0xf80000))/blue_divider; // B
	}
	else
	{
	...
	for(yy=0;yy<height;yy++)
	{
		for(xx=0;xx<width;xx++)
		{
			...
			for(k=0; k < DIRS; k++){
				...
				ny = yy + dy[k];
				if(ny < 0 || ny > 240-1 || nx < 0 || nx > 320-1) continue;
				r += weight[k]*(int)fp[ny*width+nx];
			}
			for(k=3; k < 2*DIRS; k++){
				nx = xx + dx[k];
				ny = yy + dy[k];
				if(ny < 0 || ny > 240-1 || nx < 0 || nx > 320-1) continue;
				r += weight[2*k]*(int)fp[ny*width+nx];
			}
			Lcd_Put_Cam_Pixel(xx+x,yy+y,r);
		}
	}

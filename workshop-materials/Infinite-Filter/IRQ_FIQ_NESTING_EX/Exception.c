...
void Key_EINT4_7_ISR(void) __attribute__ ((interrupt ("IRQ")));
...
volatile void * FIQ_ISR_Vector = (void *)0;

void FIQ_Select(int id, int fiq)
{
	rINTMOD = 0x0;
	...
	{
		rINTMOD = 1<<id;
		FIQ_ISR_Vector = (void *)ISR_Vectors[id];
	}
	else
	{
		FIQ_ISR_Vector = (void *)0;
	}
}

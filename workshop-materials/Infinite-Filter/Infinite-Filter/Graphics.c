...
void Lcd_Draw_Cam_Image_Outline_Filter(int x, int y, const unsigned short *fp, int width, int height){
	...
	register int xx, yy, k, pixel,r,g,b,nx,ny;
...
void Lcd_Draw_Cam_Sepia_Mode(int x, int y, const unsigned short *fp, int width, int height){
	...
	for(yy=0;yy<height;yy++)
	{
		...
		{
			pixel = (int)fp[yy*width+xx];
			r = ((pixel >> 11) & 0x1f); g = ((pixel >> 6)& 0x1f); b = ((pixel) & 0x1f);
			sr = (int)(393 * r + 769 * g + 189 * b)/1000;
			sg = (int)(349 * r + 686 * g + 168 * b)/1000;
			sb = (int)(272 * r + 534 * g + 131 * b)/1000;
			(sr > 255) ? (r = 255) : (r = sr);
			(sg > 255) ? (g = 255) : (g = sg);
			(sb > 255) ? (b = 255) : (b = sb);
			pixel = (r << 11) | (g << 5) | (b);
